...
test_predicted_json <- fromJSON(file="Data/image-level-split/test_detections.json")
...
p_both <- test3 %>%
  ...
  stat_smooth(aes(x = actual_count, y = pred_count_limited), color = "#F8766D", method = "lm", fullrange = TRUE, se = FALSE, linewidth = 1.2) + 
  geom_count(aes(x = actual_count, y = pred_count_full, color = "Raw"), alpha = 0.2) + 
  stat_smooth(aes(x = actual_count, y = pred_count_full), color = "#00BFC4", method = "lm", alpha = 0.3, fullrange = TRUE, se = FALSE, linewidth = 0.2) + 
  geom_abline(slope = 1, intercept = 0, color = "grey") +
  labs(x = "Ground Truth Counts",
       y = "Model Output Counts") +
  theme_bw()

p_both$labels$colour = "Model Output"
...
lmRawO <- lm(pred_count ~ actual_count, test2_class_other)
lmRawO
...
lmAdjustedO <- lm(pred_count ~ actual_count, test1_class_other)
lmAdjustedO
summary(lmAdjustedO)$r.squared
...
p6 <- test3_class %>%
  ...
  stat_smooth(aes(x = actual_count, y = pred_count_full), color = "#00BFC4", method = "lm", se = FALSE, linewidth = 0.2) +
...
p6$labels$colour = "Model Output"
p6$labels$size = "Number of Tiles"

p6
ggsave("eSeal_model_compared_to_truth.png", width = 9, height = 10)
